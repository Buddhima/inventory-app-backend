...
/**
 * Generic GET request to WorkflowMax API
 */
async function getFromWorkflowMax(endpoint) {
  try {
    const API_TOKEN = await getWfmToken();
...
async function getJobDetails(jobUuid) {
  return getFromWorkflowMax(`jobs/${jobUuid}`);
}

async function getJobCosts(jobUuid) {
  return getFromWorkflowMax(`jobs/${jobUuid}/costs`);
}

module.exports = { createJob, createJobCosts, getJobDetails, getJobCosts };
